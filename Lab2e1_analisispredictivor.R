...
arima_model <- auto.arima(gas_log)
summary(arima_model)
...
#Devolvemos los resultados a escala normal con una función exponencial
forecasted$mean <- exp(forecasted$mean)
forecasted$lower <- exp(forecasted$lower)
forecasted$upper <- exp(forecasted$upper)

# Visualización con ggplot2
autoplot(gas) + autolayer(exp(fitted(arima_model)), series="Ajustado") +
  autolayer(forecasted$mean, series="Pronóstico") +
...
###Métrica RMSE
rmse <- sqrt(mean((gas_log - fitted_values)^2))
###Métrica MAE
mae <- mean(abs(gas_log - fitted_values))
